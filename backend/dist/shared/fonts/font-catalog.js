...
exports.FONT_PAIRINGS = exports.FONT_CATEGORY_LABELS = void 0;
exports.getFontPairing = getFontPairing;
exports.listFontPairingIds = listFontPairingIds;
exports.buildFontCatalogPromptSection = buildFontCatalogPromptSection;
exports.inferFontPairingFromBrand = inferFontPairingFromBrand;
exports.FONT_CATEGORY_LABELS = {
    modern: 'Hiện đại',
    classic: 'Cổ điển',
    creative: 'Sáng tạo',
    future: 'Tương lai',
...
const pairingMap = new Map(exports.FONT_PAIRINGS.map((item) => [item.id, item]));
function getFontPairing(id) {
    return pairingMap.get(String(id ?? '').trim()) ?? exports.FONT_PAIRINGS[0];
}
function listFontPairingIds() {
    return exports.FONT_PAIRINGS.map((item) => item.id);
}
function buildFontCatalogPromptSection() {
    return exports.FONT_PAIRINGS.map((item) => `- ${item.id} [${exports.FONT_CATEGORY_LABELS[item.category]}]: display="${item.displayFont}" body="${item.bodyFont}" | ${item.tags.join(', ')}`).join('\n');
}
function inferFontPairingFromBrand(input) {
    const context = `${input.brand_style} ${input.personality_traits.join(' ')} ${input.occupation} ${input.description}`.toLowerCase();
    let best = exports.FONT_PAIRINGS[0];
    let bestScore = -1;
    for (const pairing of exports.FONT_PAIRINGS) {
        let score = 0;
        for (const tag of pairing.tags) {
            if (context.includes(tag))
                score += 2;
        }
        if (pairing.category === 'creative' && /sáng tạo|creative|art|design|vẽ|music|ca sĩ/.test(context))
            score += 3;
        if (pairing.category === 'classic' && /cổ điển|elegant|luxury|fashion|viết|blog/.test(context))
            score += 3;
        if (pairing.category === 'future' && /tech|gaming|ai|developer|công nghệ|game/.test(context))
            score += 3;
        if (pairing.category === 'logo' && /brand|logo|marketing|quảng cáo/.test(context))
            score += 2;
        if (pairing.category === 'modern' && score === 0)
            score += 1;
        if (score > bestScore) {
            bestScore = score;
            best = pairing;
