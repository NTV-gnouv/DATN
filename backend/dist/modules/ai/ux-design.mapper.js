...
exports.normalizeUxDesignProfile = normalizeUxDesignProfile;
exports.mapUxDesignToPage = mapUxDesignToPage;
exports.buildFallbackUxDesign = buildFallbackUxDesign;
const font_catalog_1 = require("../../shared/fonts/font-catalog");
const ai_surface_styles_1 = require("../../shared/style-presets/ai-surface-styles");
...
function resolveAvatarWidthPercent(value) {
    const parsed = Number(value);
    if (!Number.isFinite(parsed)) {
        return 32;
    }
...
function resolveBorder(borderStyle, contrastHex) {
    switch (borderStyle) {
        case 'none':
            return { width: 0, style: 'none', radius: 0, color: contrastHex };
        case 'dashed':
            return { width: 2, style: 'dashed', radius: 24, color: '#2B2418' };
        case 'sharp':
            return { width: 2, style: 'solid', radius: 4, color: contrastHex };
        case 'brutal':
            return { width: 3, style: 'solid', radius: 0, color: contrastHex };
        default:
            return { width: 1, style: 'solid', radius: 14, color: contrastHex };
    }
}
function resolveShadow(shadowStyle, accentHex, contrastHex) {
    if (shadowStyle === 'none') {
...
function buildBackgroundEffect(effect, scopeClass, colors) {
    if (effect === 'none') {
...
function resolveContentBlockColors(blockStyle, brandInput) {
...
function resolveSolidCanvasColor(surfaceClass, primary) {
    switch (surfaceClass) {
...
function resolveDesignLanguageLabel(language) {
    const labels = {
        modern: 'Hiện đại',
        editorial: 'Editorial',
...
function buildInteractionCss(hoverScale, transitionMs, focusRing, scopeClass) {
...
function inferTypographyFromBrand(input) {
...
function buildFallbackUxDesign(input) {
...
function normalizeUxDesignProfile(raw, input) {
    const fallback = buildFallbackUxDesign(input);
    const payload = (raw ?? {});
    const interaction = (payload.interaction ?? {});
    const hierarchy = (payload.visual_hierarchy ?? {});
    const typographyStyle = pickEnum(payload.typography_style, ['modern', 'editorial', 'friendly', 'minimal', 'bold'], fallback.typography_style);
    const fontPairingId = pickEnum(payload.font_pairing_id, (0, font_catalog_1.listFontPairingIds)(), fallback.font_pairing_id);
    const pairing = (0, font_catalog_1.getFontPairing)(fontPairingId);
    ...
    return {
        ...
        avatar_display_style: pickEnum(payload.avatar_display_style, ['circle', 'square', 'arched', 'ring', 'horizontal'], fallback.avatar_display_style ?? 'circle'),
        avatar_size: resolveAvatarWidthPercent(payload.avatar_size),
        background_effect: pickEnum(payload.background_effect, ['none', 'sparkle', 'shimmer', 'grain'], fallback.background_effect ?? 'none'),
        content_block_style: pickEnum(payload.content_block_style, ['white', 'tinted', 'glass', 'contrast-soft'], fallback.content_block_style ?? 'white'),
...
function mapUxDesignToPage(ux, brandInput, options) {
    const primary = brandInput.color_palette.primary.hex;
    const secondary = brandInput.color_palette.secondary_1.hex;
    const accent = brandInput.color_palette.secondary_2.hex;
    const contrast = brandInput.color_palette.contrast.hex;
    const surfaceStyle = (0, ai_surface_styles_1.getAiSurfaceStyle)(ux.surface_class);
    const canvasSolid = resolveSolidCanvasColor(ux.surface_class, primary);
    const border = resolveBorder(ux.border_style, ux.border_style === 'brutal' || ux.border_style === 'dashed' ? contrast : hexToRgba(contrast, 0.35));
    ...
    const genericInteractionCss = buildInteractionCss(ux.interaction.hover_scale, ux.interaction.transition_ms, ux.interaction.focus_ring, scopeClass);
    ...
    const pageBackground = ux.background_style === 'solid'
        ? {
            mode: 'solid',
            solid: canvasSolid,
            gradient: { start: canvasSolid, end: secondary, type: 'linear' },
            imageUrl: '',
            overlayColor: '#000000',
            overlayOpacity: 0,
        }
        : ux.background_style === 'gradient'
            ? {
                mode: 'gradient',
                solid: primary,
                gradient: { start: primary, end: accent, type: 'linear' },
                imageUrl: '',
                overlayColor: '#000000',
                overlayOpacity: 0,
            }
            : {
                mode: 'image',
                solid: primary,
                gradient: { start: primary, end: secondary, type: 'linear' },
                imageUrl: options.backgroundImageUrl ?? '',
                overlayColor: '#000000',
                overlayOpacity: isDarkPrimary ? 28 : 20,
            };
    const pairing = (0, font_catalog_1.getFontPairing)(ux.font_pairing_id);
