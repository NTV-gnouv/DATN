...
const contact_forms_repository_1 = require("./contact-forms.repository");
const contact_forms_types_1 = require("./contact-forms.types");
let ContactFormsService = class ContactFormsService {
    ...
    normalizeField(raw, index) {
        const typeCandidate = String(raw.type ?? 'text');
        const type = contact_forms_types_1.CONTACT_FORM_FIELD_TYPES.includes(typeCandidate) ? typeCandidate : 'text';
        ...
        if ((type === 'select' || type === 'radio' || type === 'checkbox') && options.length === 0) {
            throw new common_1.BadRequestException(`Field "${id}" requires at least 1 option.`);
        }
        const numberModeCandidate = String(raw.numberMode ?? 'number');
        const numberMode = numberModeCandidate === 'phone' ? 'phone' : 'number';
    ...
    normalizeFields(value) {
        ...
        const fields = value.map((item, index) => {
            if (!item || typeof item !== 'object') {
                throw new common_1.BadRequestException(`Field at index ${index} is invalid.`);
            }
            return this.normalizeField(item, index);
        });
        if (fields.length === 0) {
            throw new common_1.BadRequestException('At least 1 field is required.');
        }
        const idSet = new Set();
        fields.forEach((field) => {
            if (idSet.has(field.id)) {
                throw new common_1.BadRequestException(`Duplicate field key "${field.id}".`);
            }
            idSet.add(field.id);
        });
        return fields;
    ...
    async createForm(payload) {
        ...
        if (existing) {
            throw new common_1.BadRequestException(`Form "${id}" already exists.`);
        }
        const record = {
            id,
    ...
    async submitForm(formId, payload, requestMeta) {
        ...
        const fieldLabels = Object.fromEntries(form.fields.map((field) => [field.id, field.label.trim() || field.id]));
        const record = {
            id: submissionId,
            formId: form.id,
...
exports.ContactFormsService = ContactFormsService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [contact_forms_repository_1.ContactFormsRepository])
], ContactFormsService);
