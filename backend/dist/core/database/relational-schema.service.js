...
var RelationalSchemaService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.RelationalSchemaService = void 0;
const common_1 = require("@nestjs/common");
const json_payload_util_1 = require("./json-payload.util");
const database_hardening_service_1 = require("./database-hardening.service");
let RelationalSchemaService = RelationalSchemaService_1 = class RelationalSchemaService {
    constructor() {
        this.logger = new common_1.Logger(RelationalSchemaService_1.name);
        this.hardeningService = new database_hardening_service_1.DatabaseHardeningService();
    }
    ...
    async finalizeSchema(pool) {
        await this.hardeningService.ensureEnhancements(pool);
    }
    async createAllTables(pool) {
        await pool.execute(`
    ...
    async migrateFromLegacyJson(pool) {
        ...
        for (const row of legacyPages) {
            const page = (0, json_payload_util_1.normalizeJsonPayload)(row.payload);
            const id = String(page.id ?? row.record_id);
            const editorConfig = page.editorConfig ?? {};
            const ownerId = String(page.ownerId ?? '').trim() || null;
            await pool.execute(`INSERT INTO pages (id, title, slug, username, owner_id, theme_id, status, template, theme_tokens, header_block_id, editor_config, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, COALESCE(?, CURRENT_TIMESTAMP), COALESCE(?, CURRENT_TIMESTAMP))`, [
                ...
                (0, json_payload_util_1.toJsonColumn)(page.themeTokens ?? editorConfig.themeTokens ?? null),
            ...
            for (let index = 0; index < blocks.length; index += 1) {
                ...
                await pool.execute(`INSERT INTO page_blocks (id, page_id, block_type, sort_order, visible, data)
           VALUES (?, ?, ?, ?, ?, ?)`, [
                    blockId,
                    id,
                    blockType,
        ...
        for (const row of legacyBlocks) {
            const block = (0, json_payload_util_1.normalizeJsonPayload)(row.payload);
            const id = String(block.id ?? row.record_id);
            await pool.execute(`INSERT INTO block_definitions (id, block_type, name, version, is_default, default_data, source)
         VALUES (?, ?, ?, ?, ?, ?, ?)`, [
                id,
                ...
                (0, json_payload_util_1.toJsonColumn)(block.fields ?? block),
                block.source ? String(block.source) : null,
            ]);
        }
        for (const row of legacyThemes) {
            const theme = (0, json_payload_util_1.normalizeJsonPayload)(row.payload);
            const id = String(theme.id ?? row.record_id);
            await pool.execute(`INSERT INTO themes (id, name, version, layout, source_path, preview, description, enabled, css_defaults, theme_tokens, field_schema)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                id,
                ...
                (0, json_payload_util_1.toJsonColumn)(theme.cssDefaults ?? null),
                (0, json_payload_util_1.toJsonColumn)(theme.themeTokens ?? null),
                (0, json_payload_util_1.toJsonColumn)(theme.fields ?? null),
            ]);
        }
        for (const row of legacyCustomThemes) {
            ...
            const [pageRows] = await pool.execute(`SELECT id FROM pages WHERE id = ? LIMIT 1`, [pageId]);
            if (pageRows.length === 0) {
                continue;
            }
            await pool.execute(`INSERT INTO custom_themes (id, page_id, name, version, is_default, is_active, config, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, COALESCE(?, CURRENT_TIMESTAMP), COALESCE(?, CURRENT_TIMESTAMP))`, [
                id,
                pageId,
                String(theme.name ?? id),
        ...
        for (const row of legacyPlugins) {
            const plugin = (0, json_payload_util_1.normalizeJsonPayload)(row.payload);
            const id = String(plugin.id ?? row.record_id);
            await pool.execute(`INSERT INTO plugins (id, name, version, plugin_type, entry, source_path, enabled, permissions)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
                id,
                ...
                (0, json_payload_util_1.toJsonColumn)(plugin.permissions ?? []),
            ]);
        }
        for (const row of legacyForms) {
            const form = (0, json_payload_util_1.normalizeJsonPayload)(row.payload);
            const id = String(form.id ?? row.record_id);
            await pool.execute(`INSERT INTO contact_forms (id, name, description, submit_label, success_message, status, fields, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, COALESCE(?, CURRENT_TIMESTAMP), COALESCE(?, CURRENT_TIMESTAMP))`, [
                id,
                String(form.name ?? id),
        ...
        for (const row of legacySubmissions) {
            const submission = (0, json_payload_util_1.normalizeJsonPayload)(row.payload);
            const id = String(submission.id ?? row.record_id);
            const metadata = submission.metadata ?? {};
            await pool.execute(`INSERT INTO contact_form_submissions (id, form_id, payload, ip, user_agent, page_url, field_labels, submitted_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, COALESCE(?, CURRENT_TIMESTAMP))`, [
                ...
                (0, json_payload_util_1.toJsonColumn)(metadata.fieldLabels ?? null),
                metadata.submittedAt ? String(metadata.submittedAt) : null,
            ]);
        }
        for (const row of legacyMedia) {
            const media = (0, json_payload_util_1.normalizeJsonPayload)(row.payload);
            const id = String(media.id ?? row.record_id);
            await pool.execute(`INSERT INTO media (id, owner_id, original_name, mime_type, size, purpose, width, height, storage_key, preview_path, thumb_path, public_url, variants)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                id,
                ...
                (0, json_payload_util_1.toJsonColumn)(media.variants ?? {}),
            ]);
        }
        for (const row of legacyPageViews) {
            const event = (0, json_payload_util_1.normalizeJsonPayload)(row.payload);
            ...
            await pool.execute(`INSERT INTO page_view_events (id, page_id, slug, viewed_at, country_code, device, user_agent, referrer)
         VALUES (?, ?, ?, COALESCE(?, CURRENT_TIMESTAMP), ?, ?, ?, ?)`, [
                id,
                pageId,
                String(event.slug ?? ''),
        ...
        for (const row of legacyOnboarding) {
            const session = (0, json_payload_util_1.normalizeJsonPayload)(row.payload);
            ...
            const { id: _id, userId: _userId, pageId: _pageId, status, currentStep, startedAt, completedAt, updatedAt, ...sessionData } = session;
            await pool.execute(`INSERT INTO onboarding_sessions (id, user_id, page_id, status, current_step, session_data, started_at, completed_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, COALESCE(?, CURRENT_TIMESTAMP), ?, COALESCE(?, CURRENT_TIMESTAMP))`, [
                id,
                userId,
                pageId,
        ...
        for (const row of legacyAiChat) {
            ...
            const pageId = session.pageId ? String(session.pageId) : null;
            const { id: _id, userId: _userId, username, pageId: _pageId, status, currentStep, createdAt, updatedAt, ...sessionData } = session;
            await pool.execute(`INSERT INTO ai_chat_sessions (id, user_id, username, page_id, status, current_step, session_data, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, COALESCE(?, CURRENT_TIMESTAMP), COALESCE(?, CURRENT_TIMESTAMP))`, [
                id,
                userId,
                String(username ?? ''),
        ...
        for (const row of legacyEmailLogs) {
            const log = (0, json_payload_util_1.normalizeJsonPayload)(row.payload);
            const id = String(log.id ?? row.record_id);
            await pool.execute(`INSERT INTO email_logs (id, recipient, subject, template, status, error_message, created_at)
         VALUES (?, ?, ?, ?, ?, ?, COALESCE(?, CURRENT_TIMESTAMP))`, [
                id,
                String(log.to ?? ''),
                String(log.subject ?? ''),
        ...
        for (const row of legacyUsers) {
            const user = (0, json_payload_util_1.normalizeJsonPayload)(row.payload);
            const id = String(user.id ?? row.record_id);
            await pool.execute(`INSERT INTO user_profiles (id, name, role, deleted, metadata)
         VALUES (?, ?, ?, ?, ?)`, [
                id,
                String(user.name ?? 'User'),
                String(user.role ?? 'creator'),
                user.deleted === true ? 1 : 0,
                (0, json_payload_util_1.toJsonColumn)(user),
            ]);
        }
        await this.seedDemoData(pool);
    ...
    async seedDemoData(pool) {
        await pool.execute(`INSERT IGNORE INTO user_profiles (id, name, role) VALUES ('u-demo', 'Demo User', 'creator')`);
        await pool.execute(`INSERT IGNORE INTO themes (id, name, version, enabled)
       VALUES ('minimal', 'Minimal Theme', '1.0.0', 1), ('theme-demo', 'Minimal Theme', '1.0.0', 1)`);
        await pool.execute(`INSERT IGNORE INTO plugins (id, name, version, plugin_type, enabled)
       VALUES ('plugin-demo', 'Hero Block', '1.0.0', 'block', 1)`);
        await pool.execute(`INSERT IGNORE INTO pages (id, title, slug, username, theme_id, status)
       VALUES ('p-demo', 'My Landing Page', 'my-landing-page', 'demo', 'minimal', 'draft')`);
    }
    async readLegacyTable(pool, tableName) {
        ...
        const [rows] = await pool.execute(`SELECT record_id, payload FROM ${tableName}`);
        return rows;
    }
...
exports.RelationalSchemaService = RelationalSchemaService;
exports.RelationalSchemaService = RelationalSchemaService = RelationalSchemaService_1 = __decorate([
    (0, common_1.Injectable)()
], RelationalSchemaService);
