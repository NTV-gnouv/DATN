...
export function listFontPairingIds(): string[] {
  return FONT_PAIRINGS.map((item) => item.id);
}

export function buildFontCatalogPromptSection(): string {
  return FONT_PAIRINGS.map(
    (item) =>
      `- ${item.id} [${FONT_CATEGORY_LABELS[item.category]}]: display="${item.displayFont}" body="${item.bodyFont}" | ${item.tags.join(', ')}`,
  ).join('\n');
}

export function inferFontPairingFromBrand(input: {
  brand_style: string;
  ...
  for (const pairing of FONT_PAIRINGS) {
    ...
    for (const tag of pairing.tags) {
      if (context.includes(tag)) score += 2;
    }
    if (pairing.category === 'creative' && /sáng tạo|creative|art|design|vẽ|music|ca sĩ/.test(context)) score += 3;
    if (pairing.category === 'classic' && /cổ điển|elegant|luxury|fashion|viết|blog/.test(context)) score += 3;
    if (pairing.category === 'future' && /tech|gaming|ai|developer|công nghệ|game/.test(context)) score += 3;
    if (pairing.category === 'logo' && /brand|logo|marketing|quảng cáo/.test(context)) score += 2;
    if (pairing.category === 'modern' && score === 0) score += 1;
    if (score > bestScore) {
      bestScore = score;
      best = pairing;
    }
